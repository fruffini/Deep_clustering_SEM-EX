...
python test.py --id_exp ID_${dataset}_${ID} --redo_encoding --phase test --threshold=${th} --k_0=${k_initial} --k_fin=${k_final} --gpu_ids=${str_ids} --embedded_dimension=${emb} --AE_type=${arch} --gpu_ids=${str_ids} --verbose --dataset_name=${dataset} --data_dir=${dat} --config_dir=${cof} --reports_dir=${rep}
# Deactivate venv
deactivate