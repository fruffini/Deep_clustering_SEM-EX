...
python train.py --id_exp ID_${ID} --experiment_name experiments_stage_2 --mnist_mode=${mnist_mode} --phase pretrain --lr_pr=${lr_pr} --embedded_dimension=${emb} --dataset_name=${dataset} --n_epochs=25 --n_epochs_decay=25 --AE_type=${arch} --save_latest_freq=5000 --gpu_ids=${str_ids} --dataset_name=${dataset} --config_dir=${cof} --workdir=${workdir} --reports_dir=${rep} --shuffle_batches  --verbose
