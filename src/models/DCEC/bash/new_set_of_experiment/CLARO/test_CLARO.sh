...
python test.py --id_exp ID_${ID} --experiment_name=${exp_name} --k_0=${k_initial} --k_fin=${k_final} --embedded_dimension=${emb} --box_apply --AE_type=${arch} --gpu_ids=${str_ids} --dataset_name=${dataset} --config_dir=${cof} --workdir=${workdir} --reports_dir=${rep} --data_dir=${dat} --verbose
