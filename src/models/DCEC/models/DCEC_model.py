...
class DCECModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizers.append(self.optimizer_Pr)
            ...
            self.optimizers.append(self.optimizer_Tr)
    ...
    def init_DCEC(self):
        """Init function for DCEC models
        1) creation of the attribute self.name
        2) creation of the model folders
        3) creation of the attribute self.save_dir for the model class reference
        """
        self.name = str().join([self.name, '_', str(self.opt.num_clusters)])
        self.opt.path_man.initialize_model(model=self.name if self.opt.phase == "train" else "")
        self.save_dir = self.get_path(name='%s_dir' % (self.name if self.opt.phase == "train" else "model"))

    def set_log_losses(self):
        """create a logging file to store training losses"""

        self.acc_losses = OrderedDict({name_losses: list() for name_losses in self.loss_names})
        self.log_losses = os.path.join(self.get_path(name='logs_dir'), 'losses_{}_log_{}.txt'.format(self.opt.phase, self.opt.AE_type))
    ...
    def set_input(self, input):
        ...
        """
        tensor = input[0]
        self.x_batch = tensor.to(self.device) # copy
    ...
    def set_target_p_batch(self, ind):
        """Settings the targe tprobability respect 1 batch of data"""
    ...
    def load_model_pretrained(self):
        ...
        return self.load_networks(epoch=load_suffix, path_ext=self.get_path("weights_dir"))
    ...
    def optimize_parameters(self):
        ...
        if self.opt.phase == "pretrain":
            ...
            self.forward()
            # DCEC backward pass
            self.optimizer_Pr.zero_grad()  # set netE and netD to zero
            self.backward_DCEC()           # calculate gradients for netD and netE
            self.optimizer_Pr.step()       # update weights
        elif self.opt.phase == "train":
            # training
            # forward
            self.forward()
            # DCEC backward pass
            self.optimizer_Tr.zero_grad()  # set netE, netD and Clustering layer to zero
            self.backward_DCEC()           # calculate gradients for netD and netE
            self.optimizer_Tr.step()       # update weights
